import { getVideoIdFromLink } from './VideoBlock'

export default function VerticalVideoBlock({ link }: { link?: string }) {
  return (
    <div className='h-[600px] lg:w-[338px]'>
      {link ? (
        <iframe
          className='bg-light h-full w-full rounded-[18px]'
          src={`https://rutube.ru/play/embed/${getVideoIdFromLink(link)}`}
          allow='autoplay; fullscreen'
          allowFullScreen
          sandbox='allow-same-origin allow-scripts allow-popups allow-presentation'
          referrerPolicy='strict-origin-when-cross-origin'
        />
      ) : (
        <svg
          xmlns='http://www.w3.org/2000/svg'
          width='56'
          height='63'
          viewBox='0 0 56 63'
          fill='none'
        >
          <path
            d='M54.375 29.4516C56 30.3898 56 32.7353 54.375 33.6735L4.40625 62.523C2.78125 63.4611 0.749997 62.2884 0.749997 60.412L0.75 2.71309C0.75 0.836701 2.78125 -0.336049 4.40625 0.602145L54.375 29.4516Z'
            fill='white'
          />
        </svg>
      )}
    </div>
  )
}
